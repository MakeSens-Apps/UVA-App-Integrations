"""
last_connection — use case for GET /{id_uva}/connection.

This module holds the REAL application logic that previously lived inline in
``lambdas/uvaConnection/last_connection.py``. That legacy module is now a thin
lambda-proxy shim that re-exports ``lambda_handler`` (and the helper functions)
from here, so its observable behaviour — status codes, body shape, and the
uncaught-exception branches that become an HTTP 5xx in production (e.g. the
``getUVA: null`` -> ``None.get('createdAt')`` AttributeError) — is byte-for-byte
identical.

The same ``lambda_handler`` callable is reused by the FastAPI inbound adapter
(single HTTP Lambda) via the event_builder seam.

NOTE: ``requests`` is imported at module level on purpose; the integration tests
patch ``last_connection.requests.post`` by reference, and both the shim and this
use case share the same imported ``requests`` module object.
"""
