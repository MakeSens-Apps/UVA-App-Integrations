"""
create_racimo — use case for POST /CreateRacimo.

This module holds the REAL application logic that previously lived inline in
``lambdas/createRacimo/create_racimo.py``. That legacy module is now a thin
lambda-proxy shim that re-exports ``lambda_handler`` (and the helpers) from
here, so its observable behaviour — status codes, body shape, Content-Type
header, and the uncaught-exception branches that become an HTTP 5xx in
production — is byte-for-byte identical.

The same ``lambda_handler`` callable is reused by the FastAPI inbound adapter
(single HTTP Lambda) via the event_builder seam.

NOTE: ``requests`` and ``boto3`` are imported at module level on purpose; the
integration tests patch ``create_racimo.requests.post`` by reference, and
SigV4 signing resolves credentials through ``boto3``.
"""
